...
function renderInline(text: string, skillLevel: number | undefined, keyPrefix: string): ReactNode[] {
  ...
  while ((match = re.exec(text))) {
    ...
    parts.push(
      <span key={`${keyPrefix}-${key++}`} className="font-medium text-[var(--fu-cyan)]">
        {resolved ?? `【${inner}】`}
      </span>,
    );
...
/**
 * Renders skill/spell/benefit rules text, substituting plain 【SL】 and
 * 【SL × N】 tokens with the invested skill level when known, and
 * highlighting all bracketed formula tokens for scannability. Paragraph
