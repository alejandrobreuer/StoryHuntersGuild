...
import { STARTING_FABULA_POINTS } from "../../../lib/types";
...
function StatBlock({ title, result }: { title: string; result: StatResult }) {
  return (
    <div className="fu-panel p-4">
      <div className="flex items-baseline justify-between">
        <span className="fu-label text-[10px] text-[var(--fu-text-muted)]">{title}</span>
        <span className="fu-heading text-2xl font-bold text-[var(--fu-gold-bright)]">{result.value}</span>
      </div>
...
export function Step6DerivedStats() {
  ...
  const stats = calcDerivedStats(draft.attributes, draft.equipment, classes);

  return (
    <div className="space-y-6">
      <header>
        <h2 className="fu-heading text-2xl font-bold text-[var(--fu-gold-bright)]">Derived Stats</h2>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="fu-panel p-4">
          <div className="flex items-baseline justify-between">
            <span className="fu-label text-[10px] text-[var(--fu-text-muted)]">Fabula Points</span>
            <span className="fu-heading text-2xl font-bold text-[var(--fu-gold-bright)]">
