...
export function Step7Equipment() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h2 className="fu-heading text-3xl font-bold text-[var(--fu-gold-bright)]">Equipment &amp; Savings</h2>
      ...
      <div
        className={cn(
          "fu-panel flex flex-wrap items-center justify-between gap-4 p-5",
          overspent && "border-[var(--fu-danger)]",
        )}
      >
        <div>
          <span className="fu-label text-sm text-[var(--fu-text-muted)]">Budget</span>
          <div className="fu-heading text-2xl font-bold">
            <span className={overspent ? "text-[var(--fu-danger)]" : "text-[var(--fu-gold-bright)]"}>
              {remaining}
            </span>
            <span className="text-base text-[var(--fu-text-muted)]"> / {STARTING_BUDGET} z remaining</span>
          </div>
          {overspent && <p className="mt-1 text-sm text-[var(--fu-danger)]">Over budget — remove an item.</p>}
        </div>
        <div className="h-3 w-full flex-1 overflow-hidden rounded-full bg-[var(--fu-bg-elevated)] sm:w-48">
          <div
      ...
      <div className="fu-panel flex flex-wrap items-center gap-4 p-5">
        <button
          ...
          className="fu-label inline-flex items-center gap-2 rounded-md border border-[var(--fu-cyan-dim)] px-4 py-2.5 text-sm text-[var(--fu-cyan)] transition-colors hover:bg-[var(--fu-cyan)]/10 disabled:opacity-30"
        >
          <Dices className="h-4 w-4" /> Roll savings (2d6 × 10)
        </button>
        {draft.savingsRoll != null && (
          <span className="fu-heading text-xl font-bold text-[var(--fu-gold-bright)]">
            Rolled {draft.savingsRoll} z
          </span>
        )}
        <span className="fu-label ml-auto text-sm text-[var(--fu-text-muted)]">
          Starting zenit:{" "}
