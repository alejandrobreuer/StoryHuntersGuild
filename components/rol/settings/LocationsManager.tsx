    ...
    <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
      <div>
        ...
        <div className="surface-parchment p-5 mb-6">
          ...
          <div
            ref={(node) => { setDropRef(node); mapRef.current = node; }}
            className={cn(
              "relative w-full aspect-video bg-parchment-dark/40 border overflow-hidden mb-3 transition-colors",
              isOver ? "border-brass border-2" : "border-border"
            )}
          >
            {map?.image_url ? (
              <>
